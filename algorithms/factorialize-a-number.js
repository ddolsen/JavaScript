...
== Solution 2 ==
================
- The termination condition (or base case) is the condition upon which a recursive function/solution stops recurring. It is the problem in a recursive function that we know how to solve explicitly; the "small" problem to which we know the answer.
- If we did not have this condition, the recursion would go on until the stack space gets consumed, thereby resulting in a stack overflow
...
== Solution 3 ==
================
- Tail recursion is used to optimize memory use
- In traditional head recursion, the typical model is to perform the recursive calls first, and then take the return value of the recursive call and calculate the result. In this manner, you don't the result of the calculation until you have returned from every recursive call.
- In tail recursion, calculations are performed first, and then recursive call is executed, passing the results of the current step to the next recursive step. This results in the last statement being in the form of (return (recursive-function params)).
